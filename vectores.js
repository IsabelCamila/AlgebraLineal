class vectores {
  constructor(v1, v2) {
    ...
    if (this.n1 != 0 && this.n2 != 0) {
      this.theta = round(acos(this.a / (this.n1 * this.n2)), 3);
    } else {
      this.theta = str("división por cero")
    }
    this.aux1 = this.v1.div(this.n1);
    this.aux2 = this.v2.div(this.n2);
    this.aux1 = this.aux1.mult(3);
    this.aux2 = this.aux2.mult(3);
    this.v1 = this.v1.mult(10);
    this.v2 = this.v2.mult(10);

    this.pizarra = createGraphics(200, 200);
    this.pizarra.background(250);
    this.pizarra.noFill();
    this.pizarra.strokeWeight(2);
    this.pizarra.arc(0, 0, 210, 210, 0, HALF_PI);
    this.pizarra.fill(0);
    this.pizarra.textAlign(CENTER, BASELINE);
    this.pizarra.textSize(40);
    this.pizarra.text(this.theta, 50, 50);
  }
  mostrar() {
    strokeWeight(2);
    stroke(255, 0, 0);
    ...
    tint(255, 200);
    vertex(0, 0, 0, 0, 0);
    vertex(this.aux1.x, -this.aux1.y, this.aux1.z, 200, 0);
    vertex((this.aux1.x + this.aux2.x), -(this.aux1.y + this.aux2.y), (this.aux1.z + this.aux2.z), 200, 200);
    vertex(this.aux2.x, -this.aux2.y, this.aux2.z, 0, 200);
    vertex(0, 0, 0, 0, 0);
    endShape();
    pop();
  }
